...
static NSString *appstoreDBFileFormat = @"~/Library/klmirror-%@.db"; // Becomes ~/Library/introspy-<appName>.db
static NSString *systemDBFileFormat = @"~/Library/Preferences/klmirror-%@.db";
...
- (SQLiteStorage *)initWithDBFilePath:(NSString *) DBFilePath andLogToConsole: (BOOL) shouldLog {
    ...
    if (sqlite3_prepare_v2(dbConn, saveTracedCallStmtStr, -1, &statement, NULL) != SQLITE_OK) {
        NSLog(@"KLMirrorSQLiteStorage - Unable to prepare statement!");
        return nil;
    }
...
- (BOOL)saveTracedCall: (CallTracer*) tracedCall {
    ...
    if (argsAndReturnValueData == nil) {
        NSLog(@"KLMirrorSQLiteStorage::saveTraceCall: can't serialize args or return value");
        return NO;
    }
    ...
    if (queryResult != SQLITE_DONE) {
        NSLog(@"KLMirrorSQLiteStorage - Commit Failed: %x!", queryResult);
    	return NO;
    }
    return YES;
}
